...
- Use a signed Next.js server upload with `resource_type: "video"`; never expose the Cloudinary API secret.
...
**Original evidence:** Public demo, source repository, Cloudinary raw transcript readback, persisted vectors, 30 labeled questions, raw benchmark runs, and accuracy calculations.
